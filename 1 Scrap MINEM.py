...
for yy in range(2001,2018):
    for min in minerals:
        ...
        if (yy < 2006) and not (yy == 2005 and min == 'ORO'):
            df_temp[col_names[0]] = df_temp[col_names[0]].replace(np.nan, '', regex=True) #remove nan on first var so that we can check conditionals against it
            #get concentracion/fundicion/refineria dummies
            for i, key in enumerate(keywords):
                j = i - 1
                k = i - 2
                df_temp.loc[df_temp[col_names[0]].str.contains(key.upper()), key] = 1
            ...
            df_temp = df_temp.rename(columns = {df_temp.columns[0]: 'TITULAR'}) #else "nan" varname #1 on some dataframes causes conflicts

        else:
            df_temp.loc[df_temp.PROVINCIA == "-------", 'ETAPA'] = "CONCENTRACION"
            for key in keywords:
                df_temp[key] = 0
